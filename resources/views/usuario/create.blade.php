...
<div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <form action="{{ route('usuarios.store') }}" method="POST" class="space-y-6 bg-white p-6 rounded-lg shadow-md">
        @csrf

        <div>
            <label class="block font-semibold">Usuario</label>
            <input type="text" name="user" value="{{ old('user') }}" class="w-full border rounded px-3 py-2 mt-1" required>
        </div>

        <div>
            <label class="block font-semibold">Nombre</label>
            <input type="text" name="nombre" value="{{ old('nombre') }}" class="w-full border rounded px-3 py-2 mt-1" required>
        </div>

        <div>
            <label class="block font-semibold">Apellido</label>
            <input type="text" name="apellido" value="{{ old('apellido') }}" class="w-full border rounded px-3 py-2 mt-1" required>
        </div>
        ...
        <div>
            ...
            <input type="text" name="dpi" value="{{ old('dpi') }}" class="w-full border rounded px-3 py-2 mt-1" required>
        </div>
        ...
        <div>
            ...
            <input type="text" name="direccion" value="{{ old('direccion') }}" class="w-full border rounded px-3 py-2 mt-1">
        </div>

        <div>
            <label class="block font-semibold">Teléfono</label>
            <input type="text" name="telefono" value="{{ old('telefono') }}" class="w-full border rounded px-3 py-2 mt-1">
        </div>
        ...
        <div class="flex justify-end gap-4 mt-6">
            ...
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow">Crear Usuario</button>
        </div>
    </form>
</div>
@endsection
